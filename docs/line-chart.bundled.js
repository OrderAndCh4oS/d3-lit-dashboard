...
let LineChart = class LineChart extends s {
    constructor() {
        super();
        this.id = "line-chart";
    ...
    drawChart() {
        this.xScale.domain(extent(this.data.xs));
        const yMax = max(this.data.ys, (d) => d.length > 0 ? max(d) : undefined) || 0;
        this.yScale.domain([0, yMax]);
        const lineGenerator = line()
            .x((d) => this.xScale(d.x))
            .y((d) => this.yScale(d.y))
            .curve(this.smoothCurve ? catmullRom.alpha(0.9) : curveLinear)
        ...
        for (let index = 0; index < this.data.ys[0].length; index++) {
            const lineData = this.data.xs.map((x, i) => {
                const lineDatum = { x, y: this.data.ys[i][index] };
                if (this.data.labels?.length)
                    lineDatum.label = this.data.labels[index];
                return lineDatum;
            });
            this.svg
                .append("path")
...
LineChart.SCALE_TYPES = ["ratio", "fixed-height"];
LineChart.styles = i$2 `
    :host {
      --bg-color: #f7f7f7;
      --axis-color: #626568;
...
], LineChart.prototype, "colours", void 0);
__decorate([
    n({
        type: String,
        hasChanged(newVal, oldVal) {
            if (!LineChart.SCALE_TYPES.includes(newVal)) {
                ...
                console.warn(`Invalid scaleType value: '${newVal}'. Expected one of: ${typesStr}.`);
                return false;
            }
            return newVal !== oldVal;
        }
    })
], LineChart.prototype, "scaleType", void 0);
__decorate([
    n({ type: Number })
...
__decorate([
    n({ type: Number })
], LineChart.prototype, "strokeWidth", void 0);
__decorate([
    n({ type: Number })
], LineChart.prototype, "pointRadius", void 0);
__decorate([
    n({ type: Boolean })
], LineChart.prototype, "smoothCurve", void 0);
__decorate([
    n({ type: String })
